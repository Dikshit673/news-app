...
const NavListArr: NavListItemType[] = [
  {
    id: 1,
    name: 'Home',
    link: '/',
...
const NavListItem = ({ list, handleNavClick }: NavListItemProps) => {
  const { name, link, icon } = list;
  return (
    <li className='nav-item px-lg-2' onClick={handleNavClick}>
    ...
    <section className='nav-section'>
      <nav className='navbar navbar-expand-lg'>
        <div className='container'>
          ...
          >
            <i
              className={`${isNavOpen ? 'fa-solid fa-times' : 'fa fa-bars'}`}
            />
          </button>

          <div
            className={`collapse navbar-collapse new-back-col ${isNavOpen ? 'show' : ''}`}
          >
